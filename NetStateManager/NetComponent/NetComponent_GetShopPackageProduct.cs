...
public class NetComponent_GetShopPackageProduct : TNetComponent_SuccessOrFail<EventArg_GetShopPackageProduct,EventArg_Null,Error>
{
    public NetComponent_GetShopPackageProduct()
    {
    ...
    public override void OnEvent(EventArg_GetShopPackageProduct Arg)
    {
        NetLog.Log("NetComponent_GetShopPackageProduct : OnEvent");

        MsgReqGetShopPackageProduct pReq = new MsgReqGetShopPackageProduct();
        pReq.Uid = MyInfo.Instance.m_nUserIndex;
        pReq.I_PackageKind = Arg.m_nPackageKind;

        if(Arg.m_nPackageKind == (int)ePackageType.Chapter)
        {
            InventoryInfoManager.Instance.m_pStoreInvenInfo.ClearChapterInfoTable();
        }
        else if(Arg.m_nPackageKind == (int)ePackageType.Cycle)
        {
            InventoryInfoManager.Instance.m_pStoreInvenInfo.ClearCycleInfoTable();
        }
        else if(Arg.m_nPackageKind == (int)ePackageType.Rank)
        {
            InventoryInfoManager.Instance.m_pStoreInvenInfo.ClearRankInfoTable();
        }
        else if (Arg.m_nPackageKind == (int)ePackageType.Limit)
        {
            InventoryInfoManager.Instance.m_pStoreInvenInfo.ClearEventLimitInfoTable();
        }
        else
        {
            InventoryInfoManager.Instance.m_pStoreInvenInfo.ClearGrowthInfoTable();
        }
    ...
    {
        NetLog.Log("NetComponent_GetShopPackageProduct : RecvPacket_GetShopPackageProductSuccess");
    ...
    {
        NetLog.Log("NetComponent_GetShopPackageProduct : RecvPacket_GetShopPackageProductFailure");
