...
public class MainGame_Espresso_PVP : MainGame
{
    ...
    {
        TimeInfo.Instance.SetPvPStartTime();
        ...
        GameObject ob = Resources.Load<GameObject>("GUI/Prefabs/InGame/InGameUI_PvP");
        ...
        EventDelegateManager.Instance.OnEventInGame_OtherPlayerTurnDone += OnInGame_OtherPlayerTurnDone;
        EventDelegateManager.Instance.OnEventInGame_ChangeTurn_PvP += OnInGame_ChangeTurn_PvP;
    ...
    public override void OnSlotManagerInitializeDone()
    {
        ...
        for (int i = GameDefine.ms_nInGameSlot_X - 1; i >= 0; --i)
        {
            string strPrefabName = "BG_Character_Center_Other";

            if (i == GameDefine.ms_nInGameSlot_X - 1)
            {
                strPrefabName = "BG_Character_Left_Other";
            }
            else if (i == 0)
            {
                strPrefabName = "BG_Character_Right_Other";
            }

            int nSlotIndex = Helper.GetSlotIndex(i, GameDefine.ms_nInGameSlot_Y-1);
            Vector3 vPos = slotTable[nSlotIndex].GetPosition();
            vPos.y += 9.0f * InGameInfo.Instance.m_fInGameScale;
            GameObject ob = Resources.Load<GameObject>("2D/Prefabs/Slot_Basic/" + strPrefabName);
            ob = GameObject.Instantiate(ob);
            ...
            Plane2D pPlane2D = ob.GetComponent<Plane2D>();
            string strAtlasInfoName = "Block_BG_Opponent_Center";
            if (i == GameDefine.ms_nInGameSlot_X - 1)
            {
                strAtlasInfoName = "Block_BG_Opponent_Left";
            }
            else if (i == 0)
            {
                strAtlasInfoName = "Block_BG_Opponent_Right";
            }
            Helper.Change_Plane2D_AtlasInfo(pPlane2D, "Slot_" + pStageInfo.m_strStageTheme, strAtlasInfoName);
        }
        ...
        foreach (InGameStageMapSlotData pInGameStageMapSlotData in unitStageMapDataList)
        {
            ...
            // Create EnemyMinion
            SlotFixObject_Minion pSlotFixObject_Minion = new SlotFixObject_Minion(slotTable[nSlotIndex], pInGameStageMapSlotData.m_pInGameStageMapMetaData.m_nMetaData, pStageInfo.m_nEnemyLevel, eMinionType.EnemyMinion, eObType, eOwner.Other, false);
        ...
        for (int i = GameDefine.ms_nTeamCharacterCount - 1; i >= 0; --i)
        {
            PvPPlayerInfo pPvPPlayerInfo = InventoryInfoManager.Instance.m_pSpecialStageInvenInfo.GetPvPPlayerInfo();

            if (pPvPPlayerInfo.m_pPvPUnitInfo_byOrder.ContainsKey(i) == true)
            {
                PvPUnitInfo pPvPUnitInfo = pPvPPlayerInfo.m_pPvPUnitInfo_byOrder[i];

                if (pPvPUnitInfo != null)
                {
                    ExcelData_UnitInfo pUnitInfo = ExcelDataManager.Instance.m_pUnit.GetUnitInfo(pPvPUnitInfo.m_nUnitID);

                    if (pUnitInfo != null)
                    {
                        CharacterInvenItemInfo pInvenItemInfo = new CharacterInvenItemInfo();
                        pInvenItemInfo.m_nTableID = pPvPUnitInfo.m_nUnitID;
                        pInvenItemInfo.m_nUniqueID = GameDefine.ms_nOhterPlayerCharacterUniqueID + i;
                        pInvenItemInfo.m_nLevel = pPvPUnitInfo.m_nLevel;
                        pInvenItemInfo.m_nMaxHP = pPvPUnitInfo.m_nMaxHP;
                        pInvenItemInfo.m_nMaxSP = pPvPUnitInfo.m_nMaxSP;
                        pInvenItemInfo.m_nSp_ChargePerBlock = pPvPUnitInfo.m_nSP_ChargePerBlock;
                        pInvenItemInfo.m_nATK = pPvPUnitInfo.m_nATK;
                        pInvenItemInfo.m_nAction_Level = pPvPUnitInfo.m_nSkillLevel;
                        pInvenItemInfo.m_nPower = 0;

                        int nSlotIndex = Helper.GetSlotIndex(GameDefine.ms_nTeamCharacterCount - i - 1, GameDefine.ms_nInGameSlot_Y - 1);
                        SlotFixObject_PlayerCharacter pSlotFixObject_PlayerCharacter = new SlotFixObject_PlayerCharacter(slotTable[nSlotIndex], pUnitInfo.m_nID, GameDefine.ms_nOhterPlayerCharacterUniqueID + i, eObjectType.Character, eOwner.Other, pInvenItemInfo);
                        slotTable[nSlotIndex].AddSlotFixObject(pSlotFixObject_PlayerCharacter);
                        m_pDataStack.m_OtherPlayerCharacterTable.Add(nSlotIndex, pSlotFixObject_PlayerCharacter);
                    }
                }
            }
        }
        ...
        foreach (KeyValuePair<int, Slot> item in slotTable)
        {
            if (item.Value.GetLastSlotFixObject() == null && item.Value.IsPossibleMove() == true)
            {
                if (item.Value.GetY() == InGameInfo.Instance.m_nSlotCount_Y - 1)
                {
                    SlotFixObject pSlotFixObject = new SlotFixObject(eSlotFixObjectType.None);
                    item.Value.AddSlotFixObject(pSlotFixObject);
                }
        ...
        foreach (KeyValuePair<int, Slot> item in slotTable)
        {
            if (item.Value.GetY() == InGameInfo.Instance.m_nSlotCount_Y - 1)
            {
                item.Value.SetIntervalHeightPos(17 * InGameInfo.Instance.m_fInGameScale);
            }
        }
    ...
    public void OnInGame_Start()
    {
		...
		EspressoInfo.Instance.m_ePVP_CurrTurn_Owner = eOwner.My;
                ...
                case eObjectType.Character:
                    {
                        ...
                        {
                            if ((pPlayerCharacter.GetOwner() == eOwner.My && m_pDataStack.m_pTurnStatePattern.GetTurnState() == TurnStatePattern.eTurnState.OtherPlayer) ||
                                (pPlayerCharacter.GetOwner() == eOwner.Other && m_pDataStack.m_pTurnStatePattern.GetTurnState() == TurnStatePattern.eTurnState.Player))
                            {
                                Vector3 vPos = pSlotFixObject.GetSlot().GetPosition();
                                vPos.z = -(float)ePlaneOrder.Character_MatchDamage_Effect;
                                ParticleManager.Instance.LoadParticleSystem("FX_BlockMatchDamage", vPos).SetScale(InGameInfo.Instance.m_fInGameScale);

                                float fCorrelationRate = ExcelDataManager.Instance.m_pElement_Correlation.GetCorrelationRate(eBlockElement, pPlayerCharacter.GetUnitInfo().m_eElement);
                                ...
                                fCorrelationRate -= pPlayerCharacter.GetAdd_ElementResist(eBlockElement);
                            ...
                            if (pMinion != null)
                            {
                                switch (pMinion.GetMinionType())
                                {
                                    ...
                                    case eMinionType.PlayerSummonUnit:
                                        {
                                            m_pDataStack.m_PlayerSummonUnitTable.Remove(pMinion.GetSlot().GetSlotIndex());
                                        }
                                        break;
                                }
                            }
    ...
    public List<DamageSlotInfo> GetDamageSlotList_byLine(int nX, int nY_ToTopping)
    {
        ...
        if (EspressoInfo.Instance.m_ePVP_CurrTurn_Owner == eOwner.My)
        {
            nStart = GameDefine.ms_nInGameSlot_Y - 1;
            nEnd = nY_ToTopping + 1;
        }
        else
        {
                                ...
                                case eSlotFixObjectType.Espresso:
                                    {
                                        ...
                                        if (pEspresso != null && EspressoInfo.Instance.m_ePVP_CurrTurn_Owner != pEspresso.GetOwner() &&
                                            ...
                                            pEspresso.GetObjectType() == eObjectType.Character))
                                        {
                                            bool IsValid = true;
                                            if (pEspresso.GetObjectType() == eObjectType.Character)
                                            {
                                                SlotFixObject_PlayerCharacter pPlayerCharacter = pEspresso as SlotFixObject_PlayerCharacter;
                                                if (pPlayerCharacter != null && pPlayerCharacter.IsDead() == true)
                                                {
                                                    IsValid = false;
                                                }
                                            }
    ...
    private void ReCalculationMatchDamage()
    {
        ...
        else
        {
            ...
            foreach (KeyValuePair<int, SlotFixObject_PlayerCharacter> item in pDataStack.m_OtherPlayerCharacterTable)
            {
                pDataStack.m_nPlayerCharacterAttackValue[(int)item.Value.GetUnitInfo().m_eElement] += item.Value.GetATK();
            }
        }
        ...
        foreach (Dictionary<int, Slot> matchSlotTable in m_MatchSlotTableList)
        {
            ...
            if (pSlot_Special != null && matchNotSlotBlockList.Count != 0)
            {
                foreach (Slot pSlot in matchNotSlotBlockList)
                {
                    ...
                    if (pSlotFixObject != null && pSlotFixObject.GetOwner() != EspressoInfo.Instance.m_ePVP_CurrTurn_Owner)
                    {
                ...
                if (pSlot.GetSlotBlock() != null && SlotManager.IsCombinationBlock(pSlot) == true && pSlot.IsSlotFixObject_Obstacle() == false)
                {
                    ...
                    if (pParameta != null)
                    {
                                                ...
                                                if (pEspresso != null)
                                                {
                                                    switch (pEspresso.GetObjectType())
                                                    {
                                                        ...
                                                        case eObjectType.Character:
                                                            {
                                                                SlotFixObject_PlayerCharacter pPlayerCharacter = pEspresso as SlotFixObject_PlayerCharacter;
                                                                if (pPlayerCharacter.IsVirtualDying() == true)
                                                                {
                                                                    damageSlotInfoList.Remove(pDamageSlotInfo);
                                                                }
                                                            }
                        ...
                        if (pDamageSlotInfo != null)
                        {
                                            ...
                                            if (pEspresso != null)
                                            {
                                                switch (pEspresso.GetObjectType())
                                                {
                                                    ...
                                                    case eObjectType.Character:
                                                        {
                                                            SlotFixObject_PlayerCharacter pPlayerCharacter = pEspresso as SlotFixObject_PlayerCharacter;
                                                            int nHP = pPlayerCharacter.GetVirtualHP();
                                                            float fCorrelationRate = ExcelDataManager.Instance.m_pElement_Correlation.GetCorrelationRate(eBlockElement, pPlayerCharacter.GetUnitInfo().m_eElement);
                                                            ...
                                                            pPlayerCharacter.ChangeVirtualHP(nHP - nMatchDamage);
                            ...
                            EventDelegateManager.Instance.OnInGame_CreateBlockMatchMissile(eBlockElement, EspressoInfo.Instance.m_ePVP_CurrTurn_Owner);
                        }
                        else
                        {
                            int nY = m_pSlotManager.GetLastSlotY(pSlot.GetX(), EspressoInfo.Instance.m_ePVP_CurrTurn_Owner == eOwner.Other);
    ...
    {
        if (m_pDataStack.IsPlayerCharacterAllDead(eOwner.Other) == true)      // Other Player Check
        {
        ...
        else
        {
            ...
            EventDelegateManager.Instance.OnInGame_ShowTurnInfo_PvP(false);
        }
    }

    public void OnInGame_OtherPlayerTurnDone()
    {
        if (m_pDataStack.IsPlayerCharacterAllDead(eOwner.My) == true)       // Player Check
        {
            EventDelegateManager.Instance.OnInGame_GameOver(eGameResult.Fail);
        }
        else
        {
            EventDelegateManager.Instance.OnInGame_ShowTurnInfo_PvP(true);
        }
    }

    public void OnInGame_ChangeTurn_PvP(bool IsMyTurn)
    {
        if (!IsMyTurn)
        {
            m_pDataStack.m_pSlotManager.SetMoveAndCreateInverse(true);
            EspressoInfo.Instance.m_ePVP_CurrTurn_Owner = eOwner.Other;
            m_pTurnStatePattern.OnOtherPlayerTurn();
            EventDelegateManager.Instance.OnInGame_SlotFixObject_Cloud_Reset();
        }
        else
        {
            m_pDataStack.m_pSlotManager.SetMoveAndCreateInverse(false);
            EspressoInfo.Instance.m_ePVP_CurrTurn_Owner = eOwner.My;
            m_pTurnStatePattern.OnPlayerTurn();
            EventDelegateManager.Instance.OnInGame_SlotFixObject_Cloud_Reset();
        }
    ...
    public void OnInGame_GameOver(eGameResult eResult)
    {
        if (InGameInfo.Instance.m_eCurrGameResult == eGameResult.None)
        {
			...
			switch (eResult)
			{
				case eGameResult.Success:   Helper.OnSoundPlay("INGAME_JINGLE_VICTORY", false); break;
