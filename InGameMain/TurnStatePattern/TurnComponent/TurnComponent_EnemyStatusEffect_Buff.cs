...
public class TurnComponent_EnemyStatusEffect_Buff : TComponent<EventArg_Null, EventArg_Null>
{
    private Transformer_Timer m_pTimer_Delay_NextEvent = new Transformer_Timer();

    public TurnComponent_EnemyStatusEffect_Buff()
    {
    ...
    {
        OutputLog.Log("TurnComponent_EnemyStatusEffect_Buff : OnEvent");
        InGameTurnLog.Log("TurnComponent_EnemyStatusEffect_Buff : OnEvent");

        InGameInfo.Instance.m_eTurnComponentType = eTurnComponentType.EnemyStatusEffect_Buff;
        EventDelegateManager.Instance.OnInGame_ChangeTurnComponent(eTurnComponentType.EnemyStatusEffect_Buff);
        ...
        {
            if (pMinion.DecreaseStatusEffectTurn(eBuffType.Buff) == true)
            {
                IsProcess = true;
            }
        }
        ...
        else
        {
            ...
            OutputLog.Log("TurnComponent_EnemyStatusEffect_Buff : GetNextEvent().OnEvent(EventArg_Null.Object)");
    ...
    {
        OutputLog.Log("TurnComponent_EnemyStatusEffect_Buff : OnDone_Timer_Delay_NextEvent");
    ...
    public int IsProcess()
    {
        ...
        foreach (SlotFixObject_Minion pMinion in list)
        {
            if (pMinion.VirtualDecreaseStatusEffectTurn(eBuffType.Buff) == true)
            {
                return 1;
            }
        }
