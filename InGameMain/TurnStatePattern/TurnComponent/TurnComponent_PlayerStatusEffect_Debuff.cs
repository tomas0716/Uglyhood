...
public class TurnComponent_PlayerStatusEffect_Debuff : TComponent<EventArg_Null, EventArg_Null>
{
    private Transformer_Timer   m_pTimer_Delay_NextEvent    = new Transformer_Timer();
    private eOwner              m_eOwner                    = eOwner.My;

    public TurnComponent_PlayerStatusEffect_Debuff(eOwner eOwn)
    {
        m_eOwner = eOwn;
    ...
    {
        OutputLog.Log("TurnComponent_PlayerStatusEffect_Debuff : OnEvent");
        InGameTurnLog.Log("TurnComponent_PlayerStatusEffect_Debuff : OnEvent");

        InGameInfo.Instance.m_eTurnComponentType = eTurnComponentType.PlayerStatusEffect_Debuff;
        EventDelegateManager.Instance.OnInGame_ChangeTurnComponent(eTurnComponentType.PlayerStatusEffect_Debuff);
        ...
        foreach (SlotFixObject_Unit pUnit in list)
        {
            switch (pUnit.GetObjectType())
            {
                case eObjectType.Character:
                    {
                        ...
                        if (pPlayerCharacter != null && pPlayerCharacter.IsDead() == false && pUnit.DecreaseStatusEffectTurn(eBuffType.Debuff) == true)
                        {
                            IsProcess = true;
                        }
                    }
                ...
                default:
                    {
                        if (pUnit.DecreaseStatusEffectTurn(eBuffType.Debuff) == true)
                        {
                            IsProcess = true;
                        }
                    }
        ...
        pDataStack.m_pCloudManager.DecreaseTurn(m_eOwner == eOwner.My ? eOwner.Other : eOwner.My);
        ...
        else
        {
            ...
            OutputLog.Log("TurnComponent_PlayerStatusEffect_Debuff : GetNextEvent().OnEvent(EventArg_Null.Object)");
    ...
    {
        OutputLog.Log("TurnComponent_PlayerStatusEffect_Debuff : OnDone_Timer_Delay_NextEvent");
